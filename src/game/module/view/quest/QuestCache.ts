...
class QuestCache extends BaseCache {
	...
	public questId: number; //-1 表示任务结束 >= 0为正常id
	public progress: number;
	/**  0表示不满足接去条件 1表示接取但是未完成 2表示完成但是未提交领奖*/
	public state: number;
	...
	public update(id: number, state: number, progress: number) {
		this.questId = id;
	...
	public autoQuest() {
		if (this.questId >= 0) {
			let std: StdQuest = GameConfig.quest[this.questId];
			if (this.state == 2) {
				if (std.comp > 0) {
