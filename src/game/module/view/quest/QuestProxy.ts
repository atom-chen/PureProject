...
class QuestProxy extends BaseProxy {
	public constructor() {
		...
		this.regNetMsg(1, this.updateQuest); //请求任务列表
	}

	private updateQuest(bytes: GameByteArray) {
		let id = bytes.readShort(); //-1 表示任务结束 >= 0为正常id
		let state = bytes.readByte();
		let progress = bytes.readInt();
		GameCache.quest.update(id, state, progress);
	}

	/**提交已完成任务*/
	public sendSumitQuest() {
	...
	/**提交已完成任务*/
	public sendQuestProgress(id: number, count: number) {
